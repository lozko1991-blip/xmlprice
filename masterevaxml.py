    ...
    for prefix, url in SOURCES:
        ...
        try:
            ...
            for offer in root.xpath(".//offer"):
                ...
                # ФІКС СТРУКТУРИ LUGI: Шукаємо price строго як прямий дочірній елемент поточного offer
                p_node = offer.find('./price')
                if p_node is None or not p_node.text:
                    # Резервний пошук, якщо тег лежить в глибшій структурі всередині поточного оффера
                    p_node = offer.find('.//price')
                ...
                try:
                    ...
                    # ПРАВКА: Замість price_promo тепер записуємо old_price
                    ET.SubElement(new_off, "old_price").text = str(price)
